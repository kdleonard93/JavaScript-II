...
let caps = runners.map((upper) => {
    return allCaps.push(upper.first_name.toUpperCase());
});
...
let noneLeft = runners.filter((shirt) => {
    if (shirt.shirt_size === "L"){
    return largeShirts.push(shirt.first_name + ' ' + shirt.last_name);
    }
});
...
let tax = runners.reduce((total, cost) => {
return total += cost.donation; 
}, 0);
...
// Problem 1: Since we have a shortage of Large shrits, I will make log the amount of larger shirts just in case someone doesnt show and a size up will work.
let otherShirts = [];

let others = runners.filter((shirt) => {
    if (shirt.shirt_size != "L"){
    return otherShirts.push(shirt.first_name + ' ' + shirt.last_name);
    }
});

console.log(otherShirts);
// Problem 2: The event planner needs all of the emails listed in aplabetical order from people with dontions above $200 to send them a special thank you :). 

let bigDonations = [];

let fatChecks = runners.filter((donate) => {
    if (donate.donation >= 200){
    return bigDonations.push(donate.email);
    }
});
console.log(bigDonations.sort());
